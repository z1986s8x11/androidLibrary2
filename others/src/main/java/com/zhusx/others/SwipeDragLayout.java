...
public class SwipeDragLayout extends FrameLayout {
            ...
            case MotionEvent.ACTION_DOWN:
                if (mCacheView != null) {
                    ...
                    if (mCacheView != this) {
                        ...
                        mCacheView = null;
                        return true;
                    } else if (isOpen && mDragHelper.isViewUnder(contentView, (int) ev.getX(), (int) ev.getY())) {
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (DIRECTION_LEFT == swipeDirection) {
            //左滑
            menuView.layout(contentView.getWidth(), menuView.getTop(), contentView.getWidth() + menuView.getWidth(), menuView.getBottom());
        } else {
            //右滑
            menuView.layout(-menuView.getWidth(), menuView.getTop(), originPos.x, menuView.getBottom());
        }
    ...
    protected void onFinishInflate() {
        ...
        if (swipeDirection == DIRECTION_LEFT) {
            ...
            menuView.layout(-menuView.getWidth(), menuView.getTop(), contentView.getLeft(), menuView.getBottom());
        }
    }
