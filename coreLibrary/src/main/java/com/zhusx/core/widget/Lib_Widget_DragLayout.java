 ...
 * Created       2017/2/28 14:19
 */
public class Lib_Widget_DragLayout extends ViewGroup {
    /* 拖拽工具类 */
    ...
    public Lib_Widget_DragLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public Lib_Widget_DragLayout(Context context, AttributeSet attrs, int defStyle) {
    ...
    public void _setNextPageListener(ShowNextPageNotifier nextPageListener) {
        ...
        public boolean onTouch(View v, MotionEvent ev) {
            if (ev.getAction() == MotionEvent.ACTION_DOWN) {
                ...
                if (downY == ev.getRawY()) {
                    ...
                    // 如果的是横向拖动 则由父view处理
                    if (Math.abs(downX - ev.getRawX()) > Math.abs(downY - ev.getRawY())) {
                        v.getParent().requestDisallowInterceptTouchEvent(isInterceptHorizontalMoveEvent);
                    } else {
                        if (downY > ev.getRawY() && ViewCompat.canScrollVertically(v, 1)) {
                            v.getParent().requestDisallowInterceptTouchEvent(true);
                        } else if (downY < ev.getRawY() && ViewCompat.canScrollVertically(v, -1)) {
                            v.getParent().requestDisallowInterceptTouchEvent(true);
                        } else {
