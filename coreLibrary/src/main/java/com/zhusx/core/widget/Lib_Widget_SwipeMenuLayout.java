...
public class Lib_Widget_SwipeMenuLayout extends ViewGroup {
    ...
    //存储的是当前正在展开的View
    private static Lib_Widget_SwipeMenuLayout mViewCache;
    ...
    public Lib_Widget_SwipeMenuLayout(Context context) {
        this(context, null);
    }

    public Lib_Widget_SwipeMenuLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public Lib_Widget_SwipeMenuLayout(Context context, AttributeSet attrs, int defStyleAttr) {
     ...
     * @param ios
     */
    public Lib_Widget_SwipeMenuLayout setIos(boolean ios) {
        isIos = ios;
        return this;
    }
     ...
     * @return
     */
    public Lib_Widget_SwipeMenuLayout setLeftSwipe(boolean leftSwipe) {
     ...
     * @return
     */
    public static Lib_Widget_SwipeMenuLayout getViewCache() {
        return mViewCache;
    }
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        //左滑右滑的开关,默认左滑打开菜单
        isLeftSwipe = true;
        TypedArray ta = context.getTheme().obtainStyledAttributes(attrs, R.styleable.Lib_Widget_SwipeMenuLayout, defStyleAttr, 0);
        int count = ta.getIndexCount();
        for (int i = 0; i < count; i++) {
            int attr = ta.getIndex(i);
            //如果引用成AndroidLib 资源都不是常量，无法使用switch case
            if (attr == R.styleable.Lib_Widget_SwipeMenuLayout_lib_swipeEnable) {
                isSwipeEnable = ta.getBoolean(attr, true);
            } else if (attr == R.styleable.Lib_Widget_SwipeMenuLayout_lib_ios) {
                isIos = ta.getBoolean(attr, true);
            } else if (attr == R.styleable.Lib_Widget_SwipeMenuLayout_lib_leftSwipe) {
...
/*                    if (!mScroller.isFinished()) {
    ...
    public void smoothExpand() {
        ...
        invalidate();*/
        //展开就加入ViewCache：
        mViewCache = Lib_Widget_SwipeMenuLayout.this;
