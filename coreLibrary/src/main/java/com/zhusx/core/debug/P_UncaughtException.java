...
public final class P_UncaughtException implements UncaughtExceptionHandler {
    ...
    public void _init(Context context) {
        ...
        if (mContext == null) {
            ...
            _Files.deleteLastModifiedFile(getLogPath(context), 10);
        } else {
            if (LogUtil.DEBUG) {
    ...
    private boolean handleException(final Thread thread, final Throwable ex) {
        ...
        if (_Systems.isPermission(mContext, Manifest.permission.WRITE_EXTERNAL_STORAGE)) {
            // 收集设备参数信息
            Map<String, String> infos = collectDeviceInfo(mContext);
            // 保存日志文件
            final String log = saveCrashInfo2File(ex, infos);
            if (_Systems.isInMainThread()) {
